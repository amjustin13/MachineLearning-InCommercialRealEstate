...
# count the number of permits of each of the four categories from the Boston, Austin, and San Francisco data
...
# given the aggregated data from Boston, Austin, and San Fran, reorganize the data for training the binary classifcation model
    ...
    with open(csv_permit_file, encoding="utf8") as p_file:
        ...
        for i in range(len(x)):
            ...
            if random.uniform(0, 1) < 0.95:
                f = open(os.path.join(p_folder, "train",
                                      dir_name, f"{train_count}.txt"), 'w')
                f.write(x[i])
                f.close()
                labels_count[labels_to_dirs[y[i]]][1] += 1
            else:
                f = open(os.path.join(p_folder, "test",
                                      dir_name, f"{test_count}.txt"), 'w')
                f.write(x[i])
                f.close()
...
# given the aggregated data from Boston, Austin, and San Fran, reorganize the data for training the four categories general modoel
...
# outdated scripts for first 2 iterations of the model
    ...
    with open(x_file_path) as x_file:
        with open(y_file_path) as y_file:
            ...
            for i in range(len(x_lines)):
                ...
                if count < 12000:
                    cap = labels_total[y_lines[i]
                                       ]*.8 if labels_total[y_lines[i]] < 10000 else 10000
                    if count < cap:
                        f = open(os.path.join(p_folder, "train",
                                              dir_name, f"{count}.txt"), 'w')
                        f.write(x_lines[i])
                        f.close()
                    else:
                        f = open(os.path.join(p_folder, "test",
                                              dir_name, f"{count - cap}.txt"), 'w')
                        f.write(x_lines[i])
                        f.close()
